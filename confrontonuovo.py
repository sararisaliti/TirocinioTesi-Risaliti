...
Nx = x_train.shape[2] #Numero di feauters
...
tracker_gru = EmissionsTracker(project_name="GRU_model_selection", save_to_file=False)
tracker_gru.start()
start_gru = time()
...
best_gru = {
    ...
    "kwh": float(tracker_gru._total_energy),
...
tracker_srnn = EmissionsTracker(project_name="SRNN_model_selection", save_to_file=False)
tracker_srnn.start()
start_srnn = time()
...
best_srnn = {
    ...
    "kwh": float(tracker_srnn._total_energy),
...
tracker_esn = EmissionsTracker(project_name="ESN_model_selection", save_to_file=False)
tracker_esn.start()
start_esn = time()
...
best_esn = {
    ...
    "kwh": float(tracker_esn._total_energy),
...
tracker_eusn = EmissionsTracker(project_name="EuSN_model_selection", save_to_file=False)
tracker_eusn.start()
start_eusn = time()
...
best_eusn = {
    ...
    "kwh": float(tracker_eusn._total_energy),
...
def run_ft_model(model_name, model_builder, x_train, y_train, x_test, y_test, n_runs=3):
    ...
    for i in range(n_runs):
        ...
        #TRAINING
        tracker_tr = EmissionsTracker(project_name=f"{model_name}_Train_Run{i+1}", save_to_file=False)
        tracker_tr.start()
        t0 = time()
        model = model_builder()
        batch_size = model.bs
        ...
        results.append({
            ...
            "tempo_tr": t1 - t0,
            "tempo_ts": t3 - t2,
            "kwh_tr": float(tracker_tr._total_energy),
            "kwh_ts": float(tracker_ts._total_energy),
            "co2_tr": co2_train,
            "co2_ts": co2_test
        })
